 ...
 *   - 확인 수준  : 실제 계정 검증 기록 16개 시나리오 + 변경 경계 재검증 2건
 *   - 열린 항목  : 각 기능 문서가 이미 적어 둔 미구현·미연결 서술 + 출시 관문 문서
...
window.LAUNCH_STATUS = (function () {
  ...
  var SCOPE = {
    ...
    sealed:  { label: '봉인',       tone: 'danger', desc: '첫 출시에서 화면·링크·직접 요청을 모두 막는다. 후속 출시 대상.' },
    retired: { label: '설계 철회',  tone: '',       desc: '제품 결정으로 폐기됐다. 이 문서는 폐기된 설계의 기록이다.' },
  };

  // ── 축 2. 확인 수준 ────────────────────────────────────────────────
  var PROOF = {
    real: { label: '실계정 확인', tone: 'brand', desc: '실제 계정으로 실제 서버를 왕복해 통과한 기록이 있다.' },
    auto: { label: '자동 검증만', tone: 'info',  desc: '코드와 자동 테스트는 통과했다. 실계정·실기기·외부 서비스 확인은 없다.' },
    none: { label: '확인 없음',   tone: '',      desc: '코드가 없거나 확인 대상이 아니다.' },
  };

  // ── 현재 판정 ──────────────────────────────────────────────────────
  var verdict = [
    { label: '첫 출시(제한 공개)', value: '보류', tone: 'danger',
      why: '기능 코드는 섰지만 수정본으로 전체 사용자 흐름을 다시 통과하지 않았고, 실제 메일·로그인·푸시·서명 앱을 확인하지 않았다.' },
    { label: '전 기능 공개', value: '보류', tone: 'danger',
      why: '봉인한 후속 기능들의 실환경·법무 확인이 남아 있다.' },
    { label: '실제 결제 가동', value: '보류', tone: 'danger',
      why: '플랫폼 결제·지급·대사의 실거래 확인이 없다. 첫 출시의 유료 모임은 호스트 계좌 직접 송금이라 이 조건과 별개다.' },
  ];

  // ── 첫 출시 관문 ───────────────────────────────────────────────────
  var gates = [
    { name: '범위·정책', state: '통과', tone: 'brand',
      detail: '무엇을 팔고 무엇을 닫을지가 확정됐고 코드가 그대로 따른다. 남은 제품 결정은 1건이며 출시를 막지 않는다.' },
    { name: '기능 완성', state: '부분 통과', tone: 'warn',
      detail: '무료 모임·클럽·정기모임·신고·제재 이의는 실계정으로 통과했다. 수정본 기준 전체 16개 흐름 재실행이 남았다.' },
    { name: '운영 준비', state: '미완료', tone: 'danger',
      detail: '로그인 제공자·메일·푸시·지도·파일 저장·관리자 승인망·경보·백업 복구가 실제 환경에서 확인되지 않았다.' },
    { name: '출시후보 고정', state: '미완료', tone: 'danger',
      detail: '마지막으로 고정한 후보 뒤에 앱 수정이 들어갔다. 새 후보를 고정하고 전체 회귀를 다시 통과해야 한다.' },
  ...
  var openItems = [
    // 첫 출시를 막는 것
    { id: 'L-01', impact: 'block', kind: '검증 미실행', features: [],
      title: '수정본으로 전체 사용자 흐름을 다시 통과하지 않았다',
      detail: '변경한 두 경계(유료 모임 정책·이메일 미인증 가입)만 재실행했다. 기존 16개 흐름 전체는 남았다.', src: '출시 관문' },
    { id: 'L-02', impact: 'block', kind: '검증 미실행', features: [],
      title: '마지막 고정 출시후보 뒤에 앱 수정이 들어갔다',
      detail: '가입 직후 화면 이동 수정이 후보 뒤에 착지했다. 새 후보를 고정하고 전체 자동 회귀를 다시 통과해야 한다.', src: '출시 관문' },
    ...
    { id: 'L-09', impact: 'block', kind: '외부 준비', features: [],
      ...
      detail: '관리자 승인망 밖 차단, 장애 원격 경보, 외부 백업의 실제 복구, 롤백 훈련이 남았다.', src: '출시 관문' },
    ...
    { id: 'G-25', impact: 'limit', kind: '수동 운영', features: ['F07-09', 'F03-13'],
      title: '입금 자동 확인이 없다 — 호스트·운영자의 수동 확인이 유일한 경로다',
      detail: '첫 출시의 유료 모임 운영모델과 일치하는 상태이며, 자동 확인은 플랫폼 결제를 여는 후속 출시 조건이다.', src: '기능 문서' },
    ...
    { id: 'L-12', impact: 'block', kind: '승인 기준 미충족', features: ['F06-01', 'F06-02', 'F06-05', 'F04-14', 'F04-16', 'F07-05'],
      title: '지갑에서 돈이 나가는 실행 버튼이 화면에 그대로 남아 있다',
      detail: '결제 개통 여부를 앱이 미리 반영하는 곳은 모임 참가비와 정기모임 회비 두 흐름뿐이다. 지갑 충전, 자동 충전, 결제수단 등록, 클럽 가입비, 클럽 기부, 클럽 구독, 모임 정산의 지갑 납부, 사적 모임 호스팅비는 버튼이 그대로 보이고 누른 뒤에야 “결제 기능은 아직 준비 중입니다”로 끝난다. 안내 문구 자체는 정상이지만, 출시 승인 기준의 “포인트와 후속 기능은 고객에게 보이지 않는다” 한 줄과 어긋난다. 버튼을 감출지, 안내로 두고 기준을 고칠지 결정이 필요하다.', src: '소스 확인' },
    { id: 'G-28', impact: 'limit', kind: '개설 불가', features: ['F06-07'],
      title: '사적 모임은 첫 출시에 개설할 수 없는데 개설 버튼은 남아 있다',
      detail: '개설 방법이 세 가지인데 두 가지는 지갑에서 호스팅비를 빼고 나머지 하나는 마켓에서 산 호스팅 이용권을 쓴다. 셋 다 첫 출시에 닫혀 있어 어떤 경로로도 열 수 없다. 모임 상세의 호스팅비 결제 버튼은 그대로 보인다. 첫 출시 제품 설명에도 이 기능이 없으므로, 범위에서 명시적으로 빼고 표면을 닫을지 결정이 필요하다.', src: '소스 확인' },
    { id: 'G-29', impact: 'limit', kind: '막다른 길 잔존', features: ['F04-02', 'F04-05', 'F04-03'],
      title: '가입비를 받는 클럽은 가입이 결제 단계에서 막힌다',
      detail: '가입 확인창까지 정상 진행한 뒤 결제에서 실패한다. 다만 앱의 클럽 만들기 화면에는 가입비 입력이 없어 앱만으로는 그런 클럽을 만들 수 없다 — 운영 도구나 직접 요청으로 만들어진 경우에만 발생한다. 모임에서 닫은 “만들 수는 있는데 아무도 낼 수 없는” 구멍과 같은 모양이 클럽에는 남아 있다.', src: '소스 확인' },
    { id: 'G-30', impact: 'limit', kind: '문서 공백', features: ['F06-07'],
      title: '사적 모임 흐름에 전용 기능 문서가 없다',
      detail: '개설·지원자 선정·완료 확인·참가비 보관·분쟁까지 실제 화면과 서버 흐름이 있는데, 이 문서 모음에서는 호스팅 이용권 문서가 결제 관점으로 한 번 언급할 뿐이다. 기능 인벤토리 175개 어디에도 이 흐름의 단위가 없다.', src: '문서 점검' },
    ...
    { id: 'N-02', impact: 'next', kind: '연쇄 봉인', features: ['F04-14', 'F04-15', 'F04-16', 'F07-05', 'F06-09'],
      ...
      detail: '이 기능들은 첫 출시 범위 안에 있지만 결제 수단이 지갑 차감이라 결제 스위치와 함께 닫힌다. 정산 분담금은 계좌이체 확인 경로만 열린다. 가입비를 받는 클럽의 가입과 사적 모임 개설도 같은 이유로 닫힌다. 환불·회수·탈퇴 잔액 정리는 막지 않는다. 다만 앱이 이 축을 화면에서 미리 반영하지 않는다(L-12).', src: '결제 개통 설정' },
  ...
  var tracks = [
    { code: 'MNY-03B', name: '호스트 계좌 직접 송금', code_state: '완료', proof_state: '실환경 미확인',
      features: ['F03-13', 'F17-07', 'F07-09'], note: '첫 출시 유료 모임의 유일한 결제 방식.' },
    { code: 'P0-SCOPE-01', name: '첫 출시 범위 봉인', code_state: '완료', proof_state: '서버 확인 완료 / 서명 앱 미확인',
      features: ['F03-14', 'F03-15', 'F03-16', 'F03-17', 'F08-06', 'F09-02', 'F16-01', 'F21-01', 'F06-08'],
      note: '후속 기능의 화면·링크·직접 요청을 함께 닫는다.' },
    { code: 'ACC-LOCK-01', name: '정지 계정 권리구제', code_state: '완료', proof_state: '실계정 확인',
    ...
    { code: 'P0-E2E-01', name: '포인트 후속 이동', code_state: '완료', proof_state: '변경 경계 확인',
      features: ['F03-03', 'F06-06', 'F17-07'], note: '결제할 수 없는 유료 모임이 만들어지던 막다른 흐름을 제거했다.' },
    { code: 'P0-E2E-02', name: '메일 인증 전 제한 세션', code_state: '완료', proof_state: '변경 경계 확인',
      features: ['F01-01', 'F01-03', 'F13-02'], note: '주소를 증명하기 전에는 커뮤니티 활동을 할 수 없다.' },
    { code: 'DSP-01', name: '통합 분쟁 종결', code_state: '완료', proof_state: '실계정 왕복 미실행',
      features: ['F18-01', 'F18-02', 'F18-03', 'F18-04', 'F18-05'], note: '접수→이의→결정→보상→고지 왕복이 남았다.' },
    { code: 'CAL-01', name: '개인 가용시간·캘린더 파일', code_state: '완료', proof_state: '실기기 미확인',
      features: ['F10-01', 'F10-02', 'F10-03', 'F10-04', 'F10-05'], note: '타인 공개는 폐기했다. 실기기 가져오기가 남았다.' },
    { code: 'SAF-01', name: '신고 관할 분리와 사람 검토', code_state: '완료', proof_state: '실계정 확인',
  ...
  var scopeNote = {
    ...
    'F06-01': '잔액과 내역 조회는 열린다. 충전 버튼은 닫힌다.',
  ...
  var realProof = {
    ...
    'F03-03': '무료 모임 생성·공개, 계좌이체 모임 생성·정리',
    'F03-05': '참가 신청',
    'F03-06': '호스트 승인과 정원 반영 — 거절 경로는 미확인',
    'F04-01': '클럽 탭 진입',
 ...
 * 그 기능의 두 축과 열린 항목을 채운다. 본문은 설계 정본이고 이 띠가 현재 상태다.
 */
(function () {
  'use strict';
  function renderDomain(host, L, num, base) {
    ...
    html += pill('실계정 확인 ' + d.real + '/' + d.total, d.real ? 'info' : '');
    if (d.gaps) html += pill('열린 항목 ' + d.gaps + '건', 'warn');
  ...
  function render() {
    ...
    if (!note) {
      ...
      else if (s.scope === 'sealed') note = '첫 출시에는 화면·링크·직접 요청을 모두 막는다. 후속 출시 조건이 갖춰지면 연다.';
    }
