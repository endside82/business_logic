...
window.LAUNCH_STATUS = (function () {
  ...
  var verdict = [
    ...
    { label: '앱스토어 공개', value: '불가', tone: 'danger',
      why: '실제 외부 연동, 운영·복구 증명, 직접 송금 책임 문구와 스토어 자료가 남아 있다.' },
    { label: '플랫폼 결제', value: '후속 출시', tone: 'warn',
      why: '코드는 완료됐다. 첫 출시의 유료 모임은 호스트 계좌 직접 송금이며, 플랫폼 결제는 계약·키·실거래 증명 뒤 연다.' },
  ];

  // ── 최신 검증 결과 ────────────────────────────────────────────────
  var evidence = [
    { name: '현재 소스 고정', state: '여섯 번째 후보', tone: 'brand',
      detail: '외부 연동 서버 어댑터와 가상계좌 환불까지 포함했다. 빈 데이터베이스 설치 결과는 양쪽 281개 테이블·3,317개 컬럼·1,488개 인덱스 행·327개 외래키로 기능 차이가 없다. NICE 공개에는 별도 앱 배선과 필수 설정 기동 검증이 남아 있다.' },
    { name: '자동 회귀', state: '5개 저장소 실패 0', tone: 'brand',
      detail: '현재 후보에서 공개 API 6,824건, 앱 2,406건, 관리자 API 917건, 관리자 웹 E2E 140건을 통과했다. 실시간 서버 187건은 소스 변경이 없어 직전 결과를 유지한다.' },
  ...
  var nextActions = [
    { name: '닫힌 기능의 앱 버튼 정리',
      detail: '지갑 충전·자동 충전·카드 등록·클럽 가입비·기부·기금 인출·수익 출금·정산 지갑 납부·사적 모임 호스팅비처럼 이번 출시에 실행할 수 없는 버튼을 숨기거나, 안내 화면으로 남길 것이라면 출시 승인 기준을 명시적으로 바꾼다. 선택한 기준을 앱 회귀 테스트로 고정하면 완료다.' },
    { name: 'NICE 공개 배선 마감',
      detail: '후속 데이트 기능을 열기 전에 앱의 공급자 공통 응답, 성공·실패 콜백, 암호화 결과 전달을 연결하고 상품 ID·복귀 주소를 기동 시 검증하며 Toss·NICE 회귀를 추가한다. 이 작업은 현재 첫 출시 P0를 막지는 않는다.' },
    { name: '첫 출시 외부 계정 준비',
  ...
  var gates = [
    { name: '범위·정책', state: '부분 통과', tone: 'warn',
      detail: '서버 직접 요청과 후속 도메인의 화면·링크는 막았고 환불·해지·신고 같은 안전 출구는 유지한다. 다만 지갑에서 돈이 나가는 실행 버튼은 여전히 화면에 남아 눌린 뒤에야 거절되므로, 승인 기준의 “후속 기능 비노출”과는 아직 어긋난다.' },
    ...
    { name: '출시후보 고정', state: 'R6 소스 고정', tone: 'warn',
      detail: '여섯 번째 후보로 외부 연동 서버 어댑터와 최초 설치 정의를 고정했다. NICE 공개 배선, 코드 저장소 원격 반영과 서명 Android/iOS 산출물 고정은 별도로 확인한다.' },
  ...
  var openItems = [
    ...
    { id: 'L-12', impact: 'block', kind: '승인 기준 미충족',
      features: ['F06-01', 'F06-02', 'F06-04', 'F06-05', 'F06-09', 'F04-02', 'F04-14', 'F04-15', 'F07-05'],
      title: '이번 출시에서 닫은 결제 실행 버튼이 아직 화면에 남아 있다',
      detail: '서버는 실결제 스위치로 거절하지만 앱 버튼은 그대로 보인다 — 지갑 충전, 자동 충전, 카드 등록(2026-08-19 재개방), 클럽 가입비, 클럽 기부, 기금 인출, 수익 출금, 정산의 지갑 납부, 사적 모임 호스팅비. 누르면 “결제 기능은 아직 준비 중입니다” 안내로 끝나므로 돈이 잘못 움직이지는 않는다. 출시 범위를 미리 반영하는 곳은 모임 참가비와 정기모임 회비 두 흐름뿐이다. 승인 기준은 “포인트와 후속 기능은 고객에게 보이지 않는다”이므로, 버튼을 숨기거나 안내로 남길 정책이면 기준을 바꾸고 회귀 테스트로 고정해야 한다.', src: '2026-08-20 앱 소스 확인' },
    ...
    { id: 'N-02', impact: 'next', kind: '5차 해제', sealClass: '선행 기능 의존', features: ['F04-14', 'F04-15', 'F04-16', 'F07-05', 'F06-09'],
      title: '지갑 의존 기능 — 클럽 기부·기금 인출·클럽 구독·정산 지갑 납부·수익 출금',
      detail: '각 기능 코드가 없어서가 아니라 공통 지갑 차감이 닫혀 함께 멈춘 상태다. 계좌이체 정산과 환불·회수·탈퇴 잔액 정리는 계속 열린다.',
      codeState: '기능 코드 존재', opsState: '운영 화면 존재',
      opsDetail: '클럽 상세의 기부·인출·구독, 정산·출금 관리자 화면과 공통 돈 흐름·대사 화면이 존재한다.',
      sealReasons: ['선행 기능 의존'],
      gate: 'N-01의 money.live가 닫혀 WalletSpendService가 신규 지갑 차감을 공통 거부한다. 기능에 따라 개인구독·제공자 출시 범위도 함께 적용된다.',
      unsealSteps: ['N-01 플랫폼 결제 기반을 먼저 해제한다.', '기부·인출·구독·정산·수익 출금을 각각 별도 실계정 시나리오로 실행하고 운영 화면의 조회·거절·재처리·종결을 확인한다.', '통과한 기능만 capability와 앱 버튼을 함께 열고, 실패한 기능은 나머지와 독립적으로 봉인을 유지한다.'],
    ...
    { id: 'N-04', impact: 'next', kind: '3차 해제', sealClass: '구현·운영 잔여', features: ['F09-02', 'F09-03', 'F09-04', 'F09-05', 'F09-06', 'F09-07', 'F09-08'],
      title: '데이트 — 성인 확인·프로필·매칭·채팅·신고',
      detail: '프로필·사진·5인 신고 비노출·관리자 사람 검토는 구현됐다. NICE 서버 표준창 어댑터 뒤의 앱 콜백과 필수 설정 검증은 실제 코드 잔여다.',
      codeState: '부분 완료', opsState: '운영 화면 부분',
      opsDetail: '데이트 사진 신고의 원본 열람·개별 판정·감사는 존재한다. NICE 본인확인 공급자 상태와 실패 세션을 운영자가 확인하는 전용 화면·경보는 확인되지 않았다.',
      sealReasons: ['코드 잔여', '실신원·실기기', '안전 운영 훈련'],
      gate: 'RELEASE_SCOPE_ENABLED_DOMAINS에 DATING이 없다. 차단 목록·가려진 본인 사진 교체·본인확인은 안전 경로로 분리돼 있다.',
      unsealSteps: ['앱을 공급자 공통 시작 응답·성공/실패 콜백·NICE 암호화 결과 확인 요청으로 바꾸고 상품 ID·복귀 주소 누락을 기동 단계에서 거부한다.', '운영자가 본인확인 공급자 상태·실패 세션·재시도 결과를 확인할 운영 표면과 감사를 마련한다.', '실신원, 두 기기 realtime, 5계정 동시 신고, 관리자 복원/거절, 적법한 경찰 로그 요청 훈련을 같은 출시후보에서 통과한 뒤 DATING을 연다.'],
      doneWhen: '코드 잔여 0 + 본인확인/사진/채팅/차단/경찰 요청의 고객·운영 왕복 전부 통과', src: 'DAT-01·외부 연동 종결 현황·소스 확인' },
    ...
    { id: 'N-07', impact: 'next', kind: '1차 해제', sealClass: '실기기 검증 대기', features: ['F03-14', 'F03-15', 'F03-16', 'F03-17'],
      title: '버스·카풀 — 좌석·배차·참석자 이동 조율',
      detail: '좌석 3모드, 운전자·탑승자 상태기계, 호스트 재배정, 안전 신고와 알림 7종이 구현됐다. 과거 문서의 “알림과 운영 화면 없음”은 현재 소스와 맞지 않는다.',
      codeState: '핵심 코드 완료', opsState: '운영 화면 존재',
      opsDetail: '호스트 이동수단 설정·버스 좌석표·카풀 운전자/탑승자 운영과 좌석·카풀 신고 화면이 존재하고, 플랫폼 신고 운영 화면으로 이어진다.',
      sealReasons: ['실기기 검증', '책임 고지 승인'],
      gate: 'RELEASE_SCOPE_ENABLED_DOMAINS에 TRANSPORT가 없다. 안전 신고와 이탈 경로만 예외로 열린다.',
      unsealSteps: ['호스트 본인 좌석 선택을 이번 개통 범위에서 제외할지 구현할지 확정하고 화면 문구를 맞춘다.', '호스트·운전자·탑승자 실제 계정과 기기로 좌석 이동·재배정·제안·승인/거절/취소·알림 7종·신고를 왕복한다.', '자기책임형 이동 고지를 승인하고 TRANSPORT를 출시 범위에 추가한다.'],
  ...
  var tracks = [
    ...
    { code: 'DAT-01', name: '데이트 안전 운영', code_state: '프로필 완료·NICE 공개 배선 잔여', proof_state: '실신원·두 기기 미확인',
      features: ['F09-01', 'F09-05', 'F09-06', 'F09-07'], note: 'NICE 서버 표준창 어댑터는 완료됐지만 앱의 공급자 공통 콜백·완료 요청과 필수 설정 기동 검증이 남아 있다.' },
