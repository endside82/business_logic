...
window.LAUNCH_STATUS = (function () {
  ...
  var openItems = [
    ...
    { id: 'N-01', impact: 'next', kind: '봉인', features: ['F06-02', 'F06-05', 'F06-06', 'F06-04'],
      ...
      detail: '저장카드·지급·대사·가상계좌 코드는 완료됐다. 실제 계약·키·소액 실거래와 복구 증명이 끝나면 스위치로 연다. 이미 결제된 건의 환불·회수·역분개와 탈퇴 잔액 정리는 반대로 열어 둔다.', src: '제품 결정' },
    { id: 'N-02', impact: 'next', kind: '연쇄 봉인', features: ['F04-14', 'F04-15', 'F04-16', 'F07-05', 'F06-09'],
      ...
      detail: '이 기능들은 첫 출시 범위 안에 있지만 결제 수단이 지갑 차감이라 결제 스위치와 함께 닫힌다. 정산 분담금은 계좌이체 확인 경로만 열린다. 가입비를 받는 클럽의 가입과 사적 모임 개설도 같은 이유로 닫힌다. 환불·회수·탈퇴 잔액 정리는 막지 않는다. 다만 앱의 실행 버튼은 아직 이 상태를 미리 반영하지 않는다(L-12).', src: '결제 개통 설정' },
    ...
    { id: 'N-04', impact: 'next', kind: '봉인', features: ['F09-02', 'F09-03', 'F09-04', 'F09-05', 'F09-06', 'F09-07', 'F09-08'],
      title: '데이트를 열지 않는다',
      detail: '성인 확인, 두 기기 채팅, 다수 신고 경쟁, 수사 협조 대응을 실제 환경에서 훈련하지 않았다. NICE 서버 어댑터는 완료됐지만 앱 콜백과 필수 설정 기동 검증은 남아 있다. 차단 목록과 신고로 가려진 사진의 확인·교체는 안전 출구로 열어 둔다.', src: '제품 결정·소스 확인' },
...
(function () {
  ...
  function render() {
    ...
    if (!note) {
      ...
      else if (s.scope === 'sealed') note = '첫 출시에서 제공하지 않는 기능이다. 서버 실행과 앱의 신규 화면·링크가 막혀 있다.';
    }
