...
등급은 `Gap` / `Risk` / `Decision Needed` / `해소됨` / `미판정` 중 하나다. `미판정`은 아직 소스로
확인하지 않았다는 뜻이며, "다음 조치"에 무엇을 확인해야 판정되는지 적는다. 판정을 미룬 채 지시문을
등급 칸에 쓰지 않는다.
...
| Gap/Risk | 발견된 불일치, 미구현, 미검증이 숨겨지지 않고 등급이 붙어 있다. **모든 행의 등급이 판정값이며, "확정해야 한다"류의 지시문이 등급 칸이나 판단 칸에 남아 있지 않다.** |
| 출시 상태 | 고객이 체감하는 미완성 항목은 `docs/qa/launch-status.html` 등록부에도 올렸다. 이 PRD는 설계 정본이고 그 등록부가 현재 상태다. |
...
- **Gap/Risk 표의 등급·판단 칸에 결론 대신 지시문을 쓰는 것.** "실제 소스 대조 후 Gap/Risk/Decision
  Needed 중 하나로 확정"은 판정이 아니라 아직 하지 않은 작업이다. 이 패턴이 남으면 후보가 영원히
  쌓이기만 하고 닫히지 않아, 무엇이 열려 있는지 셀 수 없게 된다(2026-08-18 기준 286행 중 280행이
  이 상태였다). 판정을 못 하면 등급을 `미판정`으로 명시하고 무엇을 확인해야 판정되는지 적는다.
- **현재형 단정으로 미구현 기능을 서술하는 것.** PRD 본문의 "~한다"는 "그렇게 하기로 했다"는 뜻이므로,
  아직 코드·화면이 없으면 그 사실을 같은 문단에서 밝힌다.
