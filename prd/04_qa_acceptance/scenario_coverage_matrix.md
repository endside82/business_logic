...
2026-08-30 현재 제품 기능 175개 모두 시나리오 또는 수용 조건을 갖는다. 기능 목록에 적힌 숫자의 합은
1,403이지만 동일한 기준으로 센 통과 건수가 아니다. 상세 시나리오 문서 117개에서 식별한 제목은 1,086개이고,
49개 기능은 등록값과 제목 수가 다르다. 기능별 정의·소스·자동 테스트·통합 실행·실계정 연결 상태는
[기능별 제품 시나리오 완성도 감사](scenario_completeness_audit_2026-08-30.md)를 우선한다. 이 문서는
시나리오를 전부 복사하는 대신 새 기능이나 화면을 검토할 때 어떤 종류를 빠뜨리면 안 되는지 설명한다.

> 앱 저장소의 확장 대상 53개는 모두 실행됐지만 제품 기능 175개의 부분집합이다. 53/53은 그 목록의 작업
> 대기가 없다는 뜻이며 제품 전체 커버리지 100%가 아니다.
...
| 04 클럽 | 18 | 204 | 11.3 | 권한, 멤버십, 게시판, 기금, 구독, 레퓨테이션 |
...
| 08 플랜 마켓 | 15 | 144 | 9.6 | 초안/발행, 미리보기/공유, 구매 중복, 컬렉션, 리뷰, 환불·정산 |
...
| 15 경고 & 징계 | 9 | 44 | 4.9 | 신고 심사, 경고, 이의제기, 제재 집행 |
| 16 마일리지 | 8 | 42 | 5.3 | 정책, 적립/차감, 시즌, 랭킹, 검토 큐 |
| 17 정기모임 | 10 | 96 | 9.6 | 세션, FIXED/VARIABLE, 결제, 환불, 정산 |
| 18 분쟁 해결 | 5 | 24 | 4.8 | 직접 접수, 이의제기, 증빙, 호스트 인박스 |
