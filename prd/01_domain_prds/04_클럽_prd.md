...
> 문서 상태: **도메인 전환본**. 이 문서는 `business_logic/units/04_club/00_overview.md`와 117개 기능 PRD 전환 상태표를 묶어, 도메인 담당자가 어떤 기능 문서를 어떤 순서로 확인해야 하는지 보여준다.
...
이 도메인은 기능 PRD 16개로 구성된다. 현재 기능별 trace source는 총 66개이고, risk 후보는 총 59개다. 도메인 수준의 판단은 아래 기능별 PRD와 unit 근거를 따라가며 확정한다.
