...
|----|---|---|---|
| F02-01 | 홈 피드 메인 조회 | 추천 이벤트/예정 이벤트/인기 클럽/최신 플랜을 병렬 로드한다. 게스트는 인증 필수 추천 이벤트만 401 섹션 오류가 된다 | 홈 탭 진입, 스크롤 |
...
- **주요 화면**: `SCR-HF-001 메인 피드` (기존 이벤트·클럽·플랜 섹션 + 로그인 전용 사람 추천 섹션).
- **백엔드 엔드포인트**:
  - `GET /api/v1/events/recommend` (추천 이벤트, **인증 필수**)
...
- **선결 조건/상태**: `/home` 라우트와 세 `/api/v1/home/**` 목록은 비로그인 진입을 허용하지만 `/api/v1/events/recommend`는 인증 필수다. 위치 권한은 거부되어도 기본 피드는 렌더링된다.
- **결과 상태 변화**: 각 provider 응답을 독립 렌더링한다. 게스트는 예정 이벤트·인기 클럽·최신 플랜을 보고 추천 이벤트는 401 섹션 재시도 UI가 된다. 사람 추천·내 일정·찜은 숨겨진다.
...
## 9. 사람 추천 섹션 재실측 (2026-07-29)

홈은 로그인 사용자에게 `GET /api/v1/users/me/people-recommendations` 기반 **다시 만나면 좋을 사람** 수평 섹션을 추가로 조립한다. 응답은 최대 10명이며 `userId`, `nickname`, `avatarUrl`, `reasonCodes`, `coAttendedCount`만 포함한다.

후보는 증거등급 공동참석자이고 현재 사용자 상태·양방향 차단·`hideFromSearch`를 매 조회마다 적용한다. 정렬은 내가 남긴 다시 만나기 선택, 공동참석 횟수, 같은 클럽, 공통 관심/선호, 최근 90일 만남을 사용한다. 받은 선택과 숫자 점수는 노출하지 않고 이유 최대 2개만 보여 준다. 비로그인·빈 결과·로딩·오류에는 섹션과 여백을 모두 숨긴다.

근거: `PersonRecommendationController/Service`, Flutter `PeopleRecommendSection` 및 focused widget test.

<!-- 접근권한 감사 검증 (2026-07-02): 홈피드·검색·추천·트렌딩에서 비공개/초안/숨김 콘텐츠는 서버가 미노출 확인. 차단한 사람의 공개 이벤트는 발견 표면에 계속 노출됨(D-W4-1 결정 = 현행 유지, 기밀 누수 아님) — 이슈 없음. -->
