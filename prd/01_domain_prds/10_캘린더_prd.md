...
접근권한 전수 감사(Wave 4 — F10 캘린더)에서 확인된 동작 변경 사항을 반영한다.

### 친구공개 가용성 차단 관계 필터 (F1-avail)

`GET /users/{userId}/availabilities/friends`(친구 공개 가용성)가 `isFriend` 관계만 확인하고 차단 관계를 검사하지 않는 불일치가 있었다. 공개 가용성(`getPublicAvailabilities`)은 이미 `isBlockedEither` 선차단을 적용하고 있었으나 친구공개 경로만 누락. **수정**: 친구공개 가용성도 공개 가용성과 동일하게 차단 양방향 우선 필터 적용(한 줄 패리티 수정). 차단한 상대가 공개 시간대를 등록해도 차단 관계가 존재하면 열람 불가.

### 타인 캘린더 열람 범위 (변경 없음, 확인)

타인 캘린더 조회는 공개(PUBLIC) 일정만 응답하며 비공개·사적 일정은 서버가 필터한다. 감사 결과 이 동작은 이미 정상 구현되어 있음을 확인, 수정 없음.

### 홈/검색 발견 표면 — 차단한 사람의 공개 이벤트 노출 (D-W4-1, 현행 유지)

홈 피드·검색·추천·트렌딩에서 차단한 사람의 **공개** 이벤트는 계속 노출된다. 기밀 누수가 아닌 공개 콘텐츠이며, 차단의 적용 범위는 프로필·메시지·알림으로 한정하는 것이 의도된 설계임을 확인(D-W4-1 결정). 무수정.
