<!-- domain-source-first; updated: 2026-05-24; unit: warning -->
...
Flutter는 멤버용 7화면(메인/원장/제보 제출/내 제보/이의 제출/내 이의/호스트 출결 제보)과 운영진용 10화면(콘솔/정책/제보 검토함/제보 상세/이의 검토함/멤버 상세/제재/검토 대상/큐/감사 로그)을 모두 구현했고, enum 미러(`warning_enums.dart`)가 서버 enum과 1:1로 일치한다. 따라서 **조회·제출·운영진 워크플로의 화면-API 정합은 대체로 닫혀 있다**. 다만 (a) 정책 임계치 기본값이 Java 코드(3/6/10/15)와 DDL(1/3/5/7)에서 불일치, (b) `FORCED_REMOVE` 제재가 실제 클럽 멤버십 제거로 연결되는 후속 처리가 경고 서비스 안에 없음, (c) 스케줄러(미응답 자동 종료/이의 SLA/만료 자동 종료)가 모든 클럽을 순회하지 못하는 placeholder 상태, (d) 통계 화면이 서버의 `period` 파라미터를 전달하지 않음 등 **운영 신뢰성·정산성 Gap이 남아 있다**.
...
| 먼저 볼 기능 | 기능 | 이유 |
|---|---|---|
| [F15-08](../02_feature_prds/15_warning/F15-08_sanction-enforcement_prd.md) | F15-08. 제재 집행 | `FORCED_REMOVE`가 실제 멤버십 제거로 닫히지 않음, 환불/구독 정리 미연결 — 사용자 피해 가장 큼 |
...
| [F15-02](../02_feature_prds/15_warning/F15-02_report-submit-manage_prd.md) | F15-02. 신고 제출 & 내 신고 관리 | 익명 제보 차단 로직이 익명 reporter_user_id NULL 설계와 부분 충돌 (남용 방지 정확도) |
...
- **이벤트 도메인**: 이벤트 신청 시 `WarningSanctionService.isApplyRestricted`로 차단, 출결 이벤트(`LATE`/`SAME_DAY_CANCEL`/`NO_SHOW`/`REPEATED_CANCEL`)가 `autoGrantFromAttendance` 자동 GRANT 트리거.
...
| 기능 | 후보 수 | 대표 항목 |
|---|---:|---|
| [F15-08](../02_feature_prds/15_warning/F15-08_sanction-enforcement_prd.md) | 5 | `FORCED_REMOVE`가 실제 멤버십 제거/환불/구독 정리로 연결 안 됨 (outbox row만), 만료 스케줄러 placeholder |
...
| [F15-02](../02_feature_prds/15_warning/F15-02_report-submit-manage_prd.md) | 5 | 익명 누적 반려율 평가가 익명 row를 못 잡음, 자가 제보/중복이 REPORT_NOT_FOUND로 모호하게 매핑 |
