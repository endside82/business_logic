...
- **2026-05-22 (v4.5 W2/W3 — 이벤트 참가 선입금 결제·환불 신규 경로, 2026-07-29 현행 정정)**: 당시 `WalletService.payForApplication`과 `TransactionType.EVENT_PREPAYMENT_REFUND(26)`이 추가됐고 BANK_TRANSFER는 호스트 직접 수취라 분개하지 않았다. 이후 2026-06-06 표준화로 구식 `WalletService.pay/payForApplication` 본체는 차단됐으며, 현재 이벤트 WALLET 환불은 별도 executor가 아니라 `WalletRefundService.refundByTransaction`과 `EventRefundSettlementService` 경로로 충전 lot 복원·유료/무료 split·분개/정산 후처리를 수행한다. 이벤트 선입금 환불은 PG queue를 만들지 않고 `pgQueuedPaid=0`이다. 영향: F06-06, F06-10, F03-13.
