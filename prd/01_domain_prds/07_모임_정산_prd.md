...
- **API** (내 정산 이력):
  ...
  - `GET /api/v1/wallet/meeting-settlements` — 내가 참여한 모임 정산 목록. 2026-06-05부터 "DRAFT && 본인 share" 포함 + 비생성자 DRAFT 행 마스킹(DEC-V9). **앱 소비처 0 — 서버 선구현, 지갑 화면 후속** ([F07-10 §7-A](../02_feature_prds/07_meeting_settlement/F07-10_account-history-reputation_prd.md))
- **API** (호스트 신뢰도):
