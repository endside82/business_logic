...
>
> 2026-07-29 현재 소스 갱신: 체크인은 사용자 단위뿐 아니라 attendance row 단위로 게스트를 표시해야 한다. 호스트/공동호스트가 참석자 목록과 QR 체크인 화면에서 소유자와 동반 게스트를 구분하고, 게스트 체크인·미체크인 결과는 F03-20 노쇼 집계에서 owner 기준 패널티로 이어진다.
