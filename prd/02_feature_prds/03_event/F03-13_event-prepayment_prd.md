...
6. **환불 (사용자 취소 / 이벤트 취소 / 호스트 거부)**:
   - 사용자 자가 취소 → `EventParticipationCancellationService.cancelMyParticipation` (F03-05 `DELETE /api/v1/events/{eventId}/apply` 경로가 라우팅). 결제 상태별 분기:
     ...
     - `event_payment.PAID + WALLET` → `EventPaymentRefundService.refundByWallet` (D7 정책 통과 시 100% 환불, `Application(CANCELED)`, capacity 차감).
...
| POST | `/api/v1/events/{eventId}/applications/{applicationId}/refund-wallet` | EventPrepaymentController#refundByWallet | required (본인 또는 Host/CoHost) | WALLET 100% 환불 |
...
| `refundByWallet`(사용자 자가 취소 / 호스트 이벤트 취소) | WALLET | `recordRefund` + `recordPgRefundRequested`(PG 큐 경유 시) | 기존 RefundService 분개 패턴과 동일, type=`EVENT_PREPAYMENT_REFUND(26)` |
...
> WalletRefundExecutor는 본 W2/W3에서는 **미완** — 1차 슬라이스에서는 `EventPaymentRefundService.refundByWallet`이 `WalletService` 메서드를 직접 사용하면서 분개·lot 처리는 inline에서 수행한다. lot/PG queue/Settlement adjustment의 공통 헬퍼 추출은 후속 슬라이스(§10 참조)로 미룬다.
...
- 환불 규정 안내 모달 문구 (D7 단순화: "이벤트 시작 시점 이전엔 100% 환불, 시작 이후엔 환불 불가" — 후속 슬라이스에서 비율 정책 추가 시 문구 갱신)
...
| S2-6 | 참가자 자가 취소 (WALLET PAID 상태) | `event_payment.PAID(WALLET)`, deadline 통과 전 | `DELETE /api/v1/events/{eventId}/apply` → `cancelMyParticipation` → `refundByWallet` 100% → `event_payment.REFUNDED + Application=CANCELED + capacity--`. `PointTransaction(type=EVENT_PREPAYMENT_REFUND)` 1건 + 분개 1건. 알림 76. |
...
| 환불 정책 | 1차 정책은 단일 deadline 100% / 마감 후 0% (D7). 비율 정책은 후속 슬라이스. | `RefundDeadlinePolicy.isWithinDeadline(eventId)` 단일 분기. |
...
| 분류 | 근거 | 내용 | 다음 조치 |
|---|---|---|---|
| 후속 | PLAN.md §0.2 | `WalletRefundExecutor` 추출 미완 — 본 슬라이스에서 `refundByWallet`은 inline 분개·lot 처리. PG queue 통합/Settlement adjustment 공통화는 후속 슬라이스로 분리 | 후속 슬라이스(EventExtensions W4 또는 별도 결제 리팩터링 슬라이스)에서 `RefundService.doRefund`와 공통 헬퍼 추출 |
| 후속 | PLAN.md §2.6.1, D7 | 1차 환불 정책은 단일 deadline 100% / 마감 후 0%. GRADUATED(시간 비율) 정책은 미구현 | UI 안내 문구도 단일 deadline 기준으로 작성. 비율 정책 도입 시 `EventPrepayment.refundPolicyType` 활용 분기 추가 |
...
- **AC-06 (S2-6). 사용자 자가 취소 (WALLET PAID)**: Given `event_payment.PAID(WALLET)`. When `DELETE /api/v1/events/{eventId}/apply`. Then `cancelMyParticipation → refundByWallet` 100% + `Application=CANCELED + capacity--`. `PointTransaction(type=EVENT_PREPAYMENT_REFUND)` + 분개 1건. 알림 76.
...
| 항목 | 사유 | 후속 슬라이스 |
|---|---|---|
| WalletRefundExecutor 공통 헬퍼 추출 | 1차 슬라이스에서는 `RefundService.doRefund`와 인라인 환불을 그대로 두고, `EventPaymentRefundService.refundByWallet`만 별도 구현. 두 경로의 lot/PG queue/Settlement adjustment 공통화는 회귀 위험으로 분리 (PLAN.md §0.2 D14) | EventExtensions W4 또는 별도 결제 리팩터링 슬라이스 |
| 환불 정책 비율 계산 (GRADUATED) | D7로 1차는 단일 deadline 100% / 마감 후 0%. 시간 비율(24h/12h 등)은 회귀 검증 후 도입 | EventExtensions W5 (`RefundDeadlinePolicy` 시간 분기 + `EventPrepayment.refundPolicyType` 활용) |
