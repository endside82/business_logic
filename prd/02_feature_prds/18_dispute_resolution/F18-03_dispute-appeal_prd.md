...
이의제기 시 `DisputeAppealService.createAppeal`(:104-152)은 **대부분의 prefix에 대해 케이스 존재/가시성 검증 없이 `dispute_appeal` insert**를 수행한다. 사전 가시성 검증은 `CLUB_MEMBERSHIP_ACTION` prefix에 한해서만 수행되며(`DisputeAppealService.java:119-127` — 영향받은 멤버 본인 여부 확인), `EVENT_NO_SHOW`는 원본 서비스 sync 시 검증, 나머지 prefix는 audit-only로 검증 없음. `EVENT_NO_SHOW` source에 한해서만 `EventNoShowService#appeal` best-effort sync를 실행한다. `EVENT_NO_SHOW` 외 prefix는 **audit-only**다(`dispute_appeal` insert만 발생하고 원본 도메인 status는 변경되지 않음).
...
5. 서버 `DisputeAppealService.createAppeal(caseId, userId, param)`:
   ...
   - sourceType dispatcher: caseId prefix 파싱.
     - `EVENT_NO_SHOW` prefix → `EventNoShowService#appeal(noShowId, caseId)` best-effort REQUIRES_NEW sync. 실패해도 appeal insert는 롤백 안 됨.
     - 나머지 prefix → audit-only (insert만).
...
| `caseId` prefix = `EVENT_NO_SHOW` | 사전 검증 없음. audit insert 후 `EventNoShowService#appeal(noShowId, caseId)` REQUIRES_NEW best-effort sync. 실패해도 appeal insert 롤백 없음 | `DisputeAppealService.java:162-169` |
...
| `EVENT_NO_SHOW` 이의 | best-effort sync → `EventNoShowVo.status=APPEALED` | `NoShowStatus.APPEALED` 반영 | 노쇼 화면에서 APPEALED 표시 | 일치(best-effort) |
...
| P1 | **audit-only prefix 가시성 검증 없음**: `createAppeal`(:104-152)은 `CLUB_MEMBERSHIP_ACTION` 외 대부분의 prefix에 대해 케이스 존재/본인 가시성 검증 없이 `dispute_appeal` insert. 제3자가 임의 caseId로 appeal row를 만들 수 있음. | `DisputeAppealService.java:104-152` — CLUB_MEMBERSHIP_ACTION만 가시성 검증, 나머지 audit-only 검증 없음 | 비관련 당사자의 이의 insert 가능. 현재 UNIQUE(case_id, appellant_user_id) 제약이 1차 방어이나 case 가시성 보장은 없음 | audit-only prefix에 대해서도 case 조회 가능 여부 선검증 추가 검토 |
...
| P2 | **appeal 기한 미구현**: 서버 소스 주석에 "v1 미구현, 향후 정책 결정 필요"로 명시. CONFIRMED 노쇼에 대해 무기한 이의 가능. | `EventNoShowService.java:53-55` (노쇼 appeal 기한 미구현과 연관) | 장기간 경과 후 이의제기 가능 | 이의제기 기한(예: 30일) 서버 정책 결정 및 구현 |
...
### AC-05. EVENT_NO_SHOW best-effort sync

Given 케이스 caseId가 `EVENT_NO_SHOW:{noShowId}` 형식이다.
When 이의제기를 제출한다.
Then `dispute_appeal` insert가 성공하고 `EventNoShow.status`가 APPEALED로 전환된다. sync 실패 시에도 appeal insert는 유지된다.
...
| 정책 | 이의제기 기한 | CONFIRMED 노쇼 등 source별 이의 가능 기한 정의 |
