...
참가자 서비스비 분담 결제는 호스트가 배정된 제공자의 총 서비스비(X)를 설정하면, 확정된 참가자들이 자신의 몫을 지갑으로 납부하는 기능이다.
...
| Backend VO | `curated/vo/ParticipantAssignmentChargeStatusVo.java` | `assignmentId/providerRole/status/serviceFeeGross/myCoverageType/myPaidAmount/chargeable/suggestedShare` |
...
| Frontend Model | `data/models/curated/participant_assignment_charge_status_vo.dart` | `ParticipantAssignmentChargeStatusVo` Freezed |
...
### 시나리오 A: 호스트가 서비스비 총액을 설정하고 참가자가 예약금을 납부

1. 호스트가 배정 CONFIRMED 후 "서비스비 설정" CTA로 `POST /api/v1/events/{eventId}/assignments/{assignmentId}/service-fee` + `ServiceAssignmentServiceFeeParam(serviceFeeGross: X)` 호출.
...
4. 서버가 `ParticipantAssignmentChargeStatusVo` 목록을 반환: `chargeable=true`(status=CONFIRMED, serviceFeeGross>0, 미납, payer≠provider, cap 잔여)이면 "서비스비 납부" CTA가 활성화된다.
5. 참가자가 분담액을 입력하고 `ParticipantServiceFeeChargeScreen`에서 납부를 제출한다.
...
2. 서버가 `isEventEnded=true` + 보장모드 + caller∈(실출석−provider)이면 `suggestedShare`(서버 산정 1/N)를 반환.
3. 참가자 화면에서 `suggestedShare` 금액만 표시(클라이언트 금액 입력 UI 미제공).
4. 참가자가 "분담액 납부" 버튼 → `POST .../charge/my-share` + `Idempotency-Key` 헤더(body 없음).
5. 서버 `chargeMyShareOnEvent`: `CuratedChargeSplitCalculator`가 (실출석−provider) 균등분할 계산 → `chargeParticipant(calculatedAmount)` 내부 호출.
...
2. 서버가 `ASSIGNMENT_CHARGE_EXCEEDS_FEE` 에러 반환. Flutter 화면에서 에러 메시지 표시.
3. 참가자는 cap 잔여액 이하로만 납부 가능.
...
| 응답 | `ServiceAssignmentVo` 200 — `engagementId!=null` 배정에는 `ASSIGNMENT_FEE_AUTHORITY_CONFLICT` |
...
| Body | 없음(서버 산정) |
| 게이트 | `isEventEnded=true` · 보장모드(serviceFeeGross>0) · caller∈(실출석−provider) · CONFIRMED 상태 |
| 응답 | `ServiceAssignmentChargeResultVo(transactionId)` 200 |
...
| `chargeable` | status=CONFIRMED + serviceFeeGross>0 + caller 미납 + caller≠provider + cap 잔여 → true |
| `suggestedShare` | 회차 종료 + 보장모드 + caller∈(실출석−provider) → 서버 산정 1/N. 그 외 null |
| 돈/상태 | 변경 없음(읽기 전용) |
...
| 금액 표시 | `suggestedShare` 서버 값 그대로 표시(클라이언트 재계산 금지) |
...
| 조건 | CTA |
|---|---|
| `chargeable=true` + `suggestedShare==null` (회차 진행 중) | 분담액 직접 입력 후 납부(`/charge`) |
| `chargeable=true` + `suggestedShare!=null` (회차 종료 후) | `suggestedShare` 금액 표시 + 납부(`/charge/my-share`, 금액 입력 UI 없음) |
| `chargeable=false` + `myPaidAmount>0` | 납부 완료 상태 표시 |
| `chargeable=false` + `myPaidAmount==0` (FREE_EXCLUDED/HOST_SUBSIDIZED) | 무료처리됨 상태 표시 |

## 6. 상태/권한 매트릭스

| 사용자/상태 | 서버 근거 | 프론트 분기 | 사용자 결과 | 판단 |
|---|---|---|---|---|
| 확정 참가자 + chargeable=true | `assertEventParticipant` 통과 + status=CONFIRMED | `chargeable=true` 분기 | 납부 CTA 활성 | 일치 |
...
| 무료초대/대납 참가자 | `myCoverageType=FREE_EXCLUDED/HOST_SUBSIDIZED` | `chargeable=false` | 무료 처리 상태 표시 | 일치 |
