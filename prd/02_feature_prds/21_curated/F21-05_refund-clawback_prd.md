...
정산 전 전액환불(refund), 정산 전 부분환불(partial-refund), 정산 후 회수(clawback), 노쇼 forfeit(noshow-forfeit) 네 경로 모두 서버와 Flutter 양쪽에서 구현이 닫혀 있다. 서버는 각 경로별로 완전히 다른 비즈니스 규칙을 적용한다: refund는 coverage 행을 삭제해 재커버를 허용하고, partial-refund는 coverage/earning을 유지한 채 호스트 지갑에서 차액을 흡수하며, clawback은 제공자 지갑에서 net을 회수(부족분 미수금 처리), noshow-forfeit은 SELF_PAID coverage를 NOSHOW_FORFEIT으로 전환한다. partial-refund와 clawback은 Idempotency-Key 헤더가 필수이며, Flutter 앱은 Repository에서 자동 발급한다. 분할납부(top-up 다중행) beneficiary에 대한 partial-refund/clawback은 `ASSIGNMENT_REFUND_AMBIGUOUS`로 서버가 거부한다.
...
| Frontend Repository | `service_assignment_repository.dart` | `ClientMessageId.generate()` 자동 멱등키 |
...
| Verification | 서버 유닛 테스트 / codex 합의 PASS | 이중환불 차단·분기별 earning 처리 |
...
3. 서버 `clawbackAfterSettlement`:
   ...
   - `providerEarningPort.clawbackFromProvider`: 원 납부자에게 `charge.amount` 복원 + 제공자 지갑에서 net 회수(가능분 clamp, 부족분 = 제공자 CREATOR_RECEIVABLE 미수금)
...
| 가드 | SETTLEMENT_LOCKED·money-backed coverage·earning PAID·CLAWBACK audit 행 부재·단일 money row |
| Side effect | 원 납부자 전액 복원 + 제공자 net 회수(clamp) + 부족분 미수금 + `service_assignment_refund` INSERT(CLAWBACK) |
...
| 값 | 의미 |
|---|---|
| `PARTIAL_HOST_ABSORBED` | 정산 전 부분환불. 제공자 전액 유지, 호스트가 차액 부담 |
| `CLAWBACK` | 정산 후 회수. 제공자 net 회수(clamp), 부족분 미수금 |
...
| 멱등키 | `ServiceAssignmentRepository.partialRefund/clawback`에서 `ClientMessageId.generate()` 자동 발급 |
...
| 호스트 + SETTLEMENT_LOCKED + clawback | `clawbackAfterSettlement` | "정산후 환불" 버튼 노출 | 참가자 복원·제공자 net 회수 | 일치 |
...
|---|---|---|---|
| Idempotency-Key 필수 | Controller `@RequestHeader(required=true)` | `ClientMessageId.generate()` 자동 | 일치 |
...
Given 배정이 SETTLEMENT_LOCKED, 참가자가 10000원 납부(net 9000원 제공자 지급 완료).  
When `POST .../clawback`(Idempotency-Key 포함)을 호출한다.  
Then 참가자에게 10000원 복원, 제공자 지갑에서 9000원 회수(충분할 때). 부족 시 부족분 CREATOR_RECEIVABLE. `service_assignment_refund` CLAWBACK 행 INSERT.
