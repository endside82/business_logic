...
Flutter 측에서는 `SupportIssueApi`가 OperationalIssue 전용으로 구현되어 있고, **Inquiry 전용 Flutter 데이터 레이어·화면은 미구현**이다. 서버 4개 endpoint는 완전히 동작하지만 앱 진입점이 없다.
...
| Frontend API | **미구현** — `data/api/` 에 InquiryApi 없음 | Gap 항목 |
| Frontend Model | **미구현** — `data/models/inquiry/` 없음 | Gap 항목 |
| DB | `V1__init.sql:2385-2422` | `inquiry`, `inquiry_message` 테이블 DDL |
| Verification | **테스트 없음** | 검증 공백 |

## 3. 전체 동작 흐름

1. 사용자가 설정 > 고객센터 > 문의하기(또는 특정 기능 화면의 문의 버튼)에서 문의 작성 화면에 진입한다 — **Flutter 화면 미구현, 진입점 없음**.
2. 카테고리(`ACCOUNT/PAYMENT/EVENT/CLUB/REPORT/ETC`) + 제목 + 내용을 입력하고 선택적으로 `sourceType/sourceId`를 지정한다.
...
**현재 Flutter 측 구현 없음**. 아래는 서버 계약 기반 구현 시 권장 계약이다.

| 항목 | 현재 상태 | 권장 구현 |
|---|---|---|
| Route | 없음 | `/profile/inquiries`, `/profile/inquiries/new`, `/profile/inquiries/:id` |
| Screen | 없음 | `MyInquiriesScreen`, `InquiryCreateScreen`, `InquiryDetailScreen` |
| API | 없음 | `InquiryApi` (Retrofit) |
| Repository | 없음 | `InquiryRepository` |
| Provider | 없음 | `myInquiriesProvider`, `inquiryDetailProvider`, `inquiryMutationProvider` |
| Model | 없음 | `InquiryVo` (Freezed), `InquiryAddParam`, `InquiryMessageParam` |

> Fact: `community_app/lib/data/api/support_issue_api.dart`는 OperationalIssue 전용으로 구현되어 있으며 Inquiry endpoint가 포함되어 있지 않다.
...
|---|---|---|---|
| 문의 접수 | `POST /api/v1/inquiries` 구현됨 | **Flutter 미구현** | Gap |
| 문의 목록 | `GET /api/v1/inquiries/my` 구현됨 | **Flutter 미구현** | Gap |
| 문의 상세 | `GET /api/v1/inquiries/{id}` 구현됨 | **Flutter 미구현** | Gap |
| 메시지 추가 | `POST /api/v1/inquiries/{id}/messages` 구현됨 | **Flutter 미구현** | Gap |
| sourceType 분기 | `InquirySourceType` 4개 값, 검증 로직 완성 | **Flutter 미구현** | Gap |

## 8. Gap / Risk

| 등급 | 항목 | 근거 | 영향 | 다음 조치 |
|---|---|---|---|---|
| P0 | Flutter Inquiry 전 레이어 미구현 | `data/api/`, `data/models/inquiry/`, `presentation/` 없음 | 사용자가 1:1 문의 기능에 접근할 수 없음 | InquiryApi → InquiryRepository → Provider → Screen 구현 |
...
| 분류 | 항목 | 결정/작업 |
|---|---|---|
| 구현 | Flutter Inquiry 데이터 레이어 | InquiryApi, InquiryRepository, Provider, Screen 전체 구현 필요 |
