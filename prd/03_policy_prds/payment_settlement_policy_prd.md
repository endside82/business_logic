...
### 6.1-B refundToWallet free split 미보존 (Known Gap)

> **Gap (`WalletService.java:676` 주석)**: `refundToWallet` 계열(클럽 구독·기부·가입비 환불 등, txId 미보유 경로)은 **현재 전액 paid로 복원** — 원결제의 유료/무료 split 미보존. 원결제 txId 기반 split 보존/원 lot 복원은 `WalletRefundService#refundByTransaction` 으로 전환 예정이나 ClubMember/ClubSubscription 결제 txId 스키마 보강이 선행 필요.

이벤트 선입금(EVENT_PREPAYMENT) 환불은 `EventPaymentRefundService` 경로를 사용하며 이 이슈와 별개.
