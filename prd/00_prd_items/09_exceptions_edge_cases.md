...
원문에는 기능별 시나리오가 총 1180개 있다(시나리오 수치화된 기능 기준 — 2026-06-05 신규 기능 일부는 미수치화). 이 문서는 그 시나리오를 전부 다시 복사하는 대신, 기획자가 새 기능이나 화면을 검토할 때 어떤 종류의 시나리오를 빠뜨리면 안 되는지 검산하는 기준표다.
