...
> 갱신: 2026-07-08. 기능 수 증감 없이 현재 소스 기준 체크 포인트 갱신: 게스트 동반 예매, 궁합/핏 피드백, 플랜 에디터 preview/share, 위치 geocode/directions, 재사용 경험 루프.
...
| F01-03 | 인증 & 온보딩 | 이메일 인증 | 신규 사용자 | 재발송 제한, 만료/위조 토큰, 이미 인증된 사용자, Flutter 재발송/검증 배선 | 10 | 4 |
| F01-04 | 인증 & 온보딩 | 비밀번호 재설정 | 비로그인 사용자 | 미가입 이메일 보안 응답, 토큰 만료/재사용, 약한 비밀번호, request/confirm 화면 검증 | 10 | 5 |
...
| F03-05 | 이벤트 | 이벤트 신청 & 참석 | 참가자 | 승인제, 유료, 유료 승인제, 정원, 대기열, 취소, 자동 승격, 게스트 동반 예매 partySize/guestNames | 13 | 6 |
...
| F03-08 | 이벤트 | QR 체크인 | 참가자/호스트 | QR 발급, 스캔, 단축코드, 수동 체크인, 시간 게이트, 게스트 attendance row 표시 | 9 | 5 |
...
| F03-13 | 이벤트 | 이벤트 참가 선입금 (WALLET / BANK_TRANSFER) | 참가자/호스트 | APPROVED_PENDING_PAYMENT, paymentDueAt, BANK 입금확인/거부, 호스트 취소 환불, deactivation 차단, 게스트 증분 결제·라인 환불 | 11 | 5 |
| F03-14 | 이벤트 | 이동수단 공통 설정 | 호스트/공동 호스트 | 이동 운영, 자리 반납, 자차 허용의 독립 설정과 기존 차량 보호 | 6 | 0 |
| F03-15 | 이벤트 | 카풀·자차 | 호스트/운전자/탑승자 | 승용차 제안·승인/거절·취소, 자차 복귀·종료, 승차 위치·차량 신고 | 8 | 0 |
| F03-16 | 이벤트 | 대절 버스와 자리 배정 | 호스트/참가자 | 지정·비지정 자리, 호스트 전담·참가자 자기 선택, 이동·반납·신고 | 8 | 0 |
| F03-17 | 이벤트 | 차량 좌석 배치도 운영 | 플랫폼 운영자/호스트/참가자 | 표준 9종, 관리자 조회·복제·수정·활성화, 빈 좌석표·사용 중 변경 차단 | 4 | 0 |
...
| F05-01 | 검색 | 키워드 검색 | 탐색 사용자 | 영역별 결과, `/search/clubs`, `/search/plans`, 정렬, 무한스크롤, 결과 없음 | 8 | 4 |
| F05-02 | 검색 | 자동완성 서제스트 | 탐색 사용자 | 입력 전/입력 중, 디바운스, 최근/트렌딩 조합 | 7 | 3 |
| F05-03 | 검색 | 검색 필터 적용 | 탐색 사용자 | 필터 칩, 미리보기 카운트, GPS 권한 | 8 | 4 |
| F05-04 | 검색 | 최근 검색어 | 로그인 사용자 | 검색 결과 조회 후 history roundtrip, 개별 삭제, 전체 삭제, 재검색 | 9 | 4 |
| F05-05 | 검색 | 저장된 검색 | 로그인 사용자 | 목록/실행/삭제 취소, 저장/수정/삭제, 알림 연동, 중복 | 9 | 5 |
...
| F08-03 | 플랜 마켓 | 블록 에디터 | 크리에이터 | 블록 생성/수정/삭제, 자동저장, 삭제/복구, 타입 변환, URL bookmark 제안, 모바일 Enter 분할 | 9 | 5 |
...
| F09-06 | 프라이빗 데이팅 | 만남 제안 & 안전 흐름 | 매칭 사용자 | 제안/수락/거절, 일정/장소, 안전 기능, 완료 후 데이트 피드백 | 10 | 5 |
...
| F11-06 | 리뷰 & 신고 | 취향 평가 & 취향 프로필 | 로그인 사용자 | 이벤트 피드백, 데이트 피드백, 태그 가중치, 데이터 부족, export/delete 포함 | 8 | 5 |
| F12-01 | 알림 | 알림 목록 조회 & 읽음 관리 | 로그인 사용자 | reschedule/recap/feedback/date-meeting/dispute 딥링크, 모두 읽음, 삭제, 무한스크롤 | 6 | 4 |
...
| F14-04 | 위치 & 길찾기 | 위치 프라이버시 대시보드 | 참석자/호스트 | 이벤트별 토글, 호스트 분기, 실패 롤백, 30일 access-log 소비 | 6 | 4 |
| F14-05 | 위치 & 길찾기 | 이벤트 길찾기 | 참석자/호스트 | 현재 위치/저장 주소, 외부 지도, 참석자 거리, 좌표 null guard | 9 | 4 |
| F14-06 | 위치 & 길찾기 | 역지오코딩 | 위치 입력 사용자 | 좌표→주소, 주소→좌표 geocode, 캐시, 좌표 오류, 외부 시스템 연동 장애 | 6 | 4 |
...
| F17-02 | 정기모임 | 정기모임 상세 조회 | 사용자/호스트/게스트 | DRAFT 마스킹, isHost(JSON: host), 세션 요약, status×viewer CTA 분기, 재사용 경험 루프 진입 | 8 | 1 |
...
| F03-20 | 이벤트 | 이벤트 노쇼 관리 | 호스트/참가자 | NoShowStatus(CONFIRMED/APPEALED/OVERTURNED), 호스트·cohost 단건/일괄 확정, 참가자 소명(appealCaseId 외부 발급 필요), 호스트 뒤집기(OVERTURNED 제재 카운트 제외), 게스트 노쇼 owner 패널티, 노쇼 사후 환불(NoShowRefund + dispute_case_id 자동 생성), EventApplyRestrictionGuard 17개 진입점 통합 | — | — |
