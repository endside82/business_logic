...
| 이 문서로 정할 것 | 도메인 목적, 기능 수, 도메인 간 의존 관계 (21개 도메인) |
| 같이 볼 문서 | 01_domain_prds/, 03_information_architecture.md |

> 2026-06-05: 도메인은 7개 줄기로 묶인다(`03_information_architecture.md` §1) — 시작·발견(01/02/05), 모임 줄기(03+17·07·21), 클럽 줄기(04+16·15), 돈(06/08), 사람·관계(09/19/10/13), 신뢰·운영 횡단(11/18/20), 기반 횡단(12/14). 16 마일리지·15 경고는 클럽 단위로 설정·운영되는 클럽 줄기 위성, 17 정기모임·07 모임 정산·21 제공자 배정·정산은 모임 줄기 위성이다. 번호·ID는 안정 식별자라 유지.
...
| 21 | 제공자 배정·정산 | 호스트 기획 모임에 서비스 제공자를 배정하고 참가자 서비스비 분담 결제·제공자 정산·무료 초대·호스트 대납·환불·회수·계약금 선납·정기 묶음 배정·정산을 처리한다. 모임(이벤트) 줄기 위성이며 07 모임 정산, 17 정기모임과 같은 줄기에 속한다. | 7 |
