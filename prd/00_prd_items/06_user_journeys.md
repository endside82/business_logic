...
| 항목 | 내용 |
|---|---|
| 목적 | 기능 목록을 실제 사용자 행동 흐름으로 재구성해 시작, 판단, 성공, 실패, 복구를 한 번에 본다. |
| 보는 시점 | 화면 흐름 설계, 사용자 스토리 작성, QA 시나리오 정리 시점 |
| 이 문서로 정할 것 | 핵심 사용자 여정, 단계별 분기, 복구 지점, 다음 행동 |
| 같이 볼 문서 | 09_exceptions_edge_cases.md, 10_impact_matrix.md |
...
| 프로필 | 노출 켜짐/꺼짐, 사진 개수, 소개 입력 조건 |
| 스와이프 | 일일 좋아요 한도, 매칭 성사 모달, 후보자 없음 상태 |
...
- 여정의 시작점이 게스트, 로그인 사용자, 관계자 중 누구인지 명확해야 한다.
- 여정 중 돈, 위치, 알림, 리뷰/신뢰가 바뀌면 영향 문서와 연결한다.
- 중간 실패 시 사용자가 멈추지 않도록 재시도, 복구, 대체 경로를 둔다.
