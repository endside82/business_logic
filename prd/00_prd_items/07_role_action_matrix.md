...
정산 이의제기는 정산이 신청(활성화)된 이후부터만 가능하다 — 준비 중(DRAFT) 정산에는 서버가 이의 생성을 거부하고 앱도 버튼을 숨긴다(2026-06-05). 호스트 본인 share 이의는 상태와 무관하게 불가.
...
## 2026-07-02 접근권한 감사 교정 반영

> 전수 감사(21개 도메인·4개 웨이브, 2026-06-30~07-02)에서 확정한 역할×액션 변경. 위 매트릭스 셀 중 영향받은 항목과 새로 명시된 운영 권한을 여기에 기록한다. 상세 근거는 `docs/audit/access-control-2026-06-30/` 참조.

### 클럽 권한 매트릭스 교정

위 클럽 권한 매트릭스의 "클럽 수정" 행은 **관리자 O · 소유자 O** (기존 표기 유지). 교정 내용: 서버가 소유자 전용으로만 구현되어 있던 것을 PRD와 일치하도록 관리자∪소유자로 완화했다. 클럽 삭제·소유권 이전은 소유자 전용 유지.

| 항목 | 관리자(admin) | 소유자(owner) | 비고 |
|---|:-:|:-:|---|
| 클럽 기본정보 수정 | O | O | 서버 2026-07-02 PRD 정합 완료 |
| 클럽 삭제·소유권 이전 |  | O | 소유자 전용 유지 |
| 대기 신청자(APPLY) 명단 열람 | O | O | 운영진 전용(교정 전: 전 멤버) |
| 멤버 1인당 기부·출석 통계 | O | O | 운영진 전용(교정 전: 전 멤버) |
| 클럽 이벤트 통계 | O | O | 운영진 전용(교정 전: 전 멤버) |
| 기금 출금이력 (은행PII 포함) |  | O | 소유자 전용(교정 전: 전 멤버) |

### 경고·제재 역할 분리 (SoD)

경고·제재 파이프라인은 검토와 제재 권한이 분리됐다. 두 역할은 독립이며, 둘 다 하려면 둘 다 보유해야 한다.

| 액션 | WARNING_REVIEWER | SANCTION_PROPOSER | OWNER |
|---|:-:|:-:|:-:|
| 경고 신고·이의 열람·검토 | O |  | O |
| 경고 분쟁 케이스 상세 (당사자 외) | O |  | O |
| 제재 부과(apply) · 해제(revoke) |  | O | O |
| FORCED_REMOVE(강제 추방) |  |  | O |

### 마일리지 권한 매트릭스

세 플래그는 독립 위임. 포인트와 배지를 모두 관리하려면 두 권한을 모두 보유해야 한다.

| 액션 | REWARD_MANAGER | BADGE_AWARD_MANAGER | MILEAGE_MANAGER |
|---|:-:|:-:|:-:|
| 포인트 지급·차감·되돌림·제안승인 | O |  |  |
| 배지 부여·회수 |  | O |  |
| 마일리지 설정 관리·조회 |  |  | O |
...
| 액션 | ADMIN | 비고 |
|---|:-:|---|
| 강제환불 처리(force/uphold/evidence) | O | URL 가드 + 서비스단 ADMIN 단언 2중 |
| 환불이의·마켓 분쟁 관리 | O | 동일 |
| 환불 분쟁 조회(getOpenDisputes/getDisputeVo) | O | 심층방어 |
| 스케줄러 자동승인(actorUserId=0) | — | sentinel 예외 |
