...
이 템플릿은 전체 175개 기능 중 unit 근거를 보유한 117개 하위 집합을 Notion 카드로 옮길 때 사용한다. 나머지 58개 no-unit 기능은 source-first PRD와 실제 소스를 기준으로 같은 카드 형식에 확장한다. 원문 링크 없이도 기획자가 기능의 목적, 상태, 예외를 빠르게 파악하도록 설계했다.
