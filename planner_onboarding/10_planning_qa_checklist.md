...
| 설정 반영 | 카테고리 OFF나 방해금지 시간에 걸리면 어떻게 되는가 |
...
| 외부 시스템 | 실패 시 확인할 것 |
|---|---|
| 소셜 로그인 SDK | 사용자가 취소, 토큰 검증 실패, provider 오류 |
| 이메일 발송 | 발송 실패, 만료 링크, 재발송 제한 |
| PG/결제 | 결제 중 이탈, 콜백 지연, 중복 콜백, 승인 실패 |
| FCM/푸시 | OS 권한 거부, 토큰 만료, 다중 기기 |
| 지도/지오코딩 | 권한 거부, API 장애, 주소 없음, 외부 지도 앱 미설치 |
| 파일 업로드 | presigned URL 실패, 업로드 실패, 삭제 실패 |
...
| 정산 | DRAFT 상태인데 참가자에게 납부 요청이 보이는 경우 |
...
```text
1. 이 기능은 전체 21개 도메인 중 어디에 속하는가?
2. 175개 기능 인벤토리(117개 unit-backed + 58개 source-first) 중 기존 기능의 확장인가, 신규 기능인가?
...
```mermaid
flowchart TD
    A["서비스 온보딩 홈"] --> B["Unit-backed 서비스 지도"]
    A --> C["175개 기능 인벤토리"]
    A --> D["대표 사용자 여정"]
    A --> E["상태와 정책 가이드"]
    C --> F["업무 영역별 플레이북"]
...
처음부터 175개 기능 카드를 모두 상세화할 필요는 없다. 다만 전체 175개 인벤토리와 그 안의 117개 unit-backed / 58개 source-first 경계는 반드시 유지하고, 실제 PRD나 QA가 시작되는 기능부터 카드 상세를 채우는 방식이 누락을 가장 적게 만든다.
